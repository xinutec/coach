...
/// A mobility drill's dose. "Warm up" with no number is not an instruction.
...
/// anchor, and real weeks pull it toward the athlete's own rate, so there is no
/// cliff between "cold start" and "personal average".
...
fn mode_fit(mode: Mode, ex: &ExerciseInfo) -> f64 {
    match mode {
        ...
        // A balanced week is built on rep and weighted work, with holds as a
        // narrower accessory stimulus. A flat score would hand that decision to
        // the tie-break on exercise id.
...
/// says why.
fn loadable(ex: &ExerciseInfo, exercise_loads: &BTreeMap<ExerciseId, Vec<f64>>) -> Option<Loaded> {
    let inventory = || {
...
fn blocked_ideal(
    ...
    group_id: GroupId,
    chosen_id: ExerciseId,
) -> Option<Substitution> {
    let name_of = |ids: &[EquipmentId]| -> Vec<String> {
        ids.iter()
...
fn build_warmup(
    ...
    group_name: &BTreeMap<GroupId, String>,
    ...
    let mut load: BTreeMap<GroupId, f64> = BTreeMap::new();
    for w in work {
    ...
    let mut want: Vec<(GroupId, f64)> = load
        .into_iter()
    ...
    let drills: Vec<&ExerciseInfo> = input
        ...
        .collect();
    let primaries = |e: &ExerciseInfo| -> Vec<GroupId> {
        e.groups
            .iter()
            .filter(|(_, r)| *r == MuscleRole::Primary)
            .map(|(g, _)| *g)
...
pub fn evaluate(input: &PacingInput, now: NaiveDateTime) -> PacingNow {
    ...
    let region_of: BTreeMap<GroupId, Region> =
        input.groups.iter().map(|g| (g.id, g.region)).collect();

    let mut current: BTreeMap<GroupId, f64> = BTreeMap::new();
    let mut avg_sum: BTreeMap<GroupId, f64> = BTreeMap::new();
    // Age-weighted unrecovered load per group: a set counts fully when fresh and
    // ramps to zero over its region's recovery horizon (G6).
    let mut unrecovered: BTreeMap<GroupId, f64> = BTreeMap::new();
    ...
    let novel_introduced = {
        let mut first_seen: BTreeMap<ExerciseId, NaiveDateTime> = BTreeMap::new();
        for s in &planning {
    ...
    let next_phrase = |s: &Suggestion| -> String {
        if s.kind == SuggestionKind::Warmup {
            // Total over the ask, with no catch-all: the compiler checks every
            // shape a warm-up can be built as.
