 ...
 * The parameters make it a silent, bare form demo:
 * - `mute=1`: browsers refuse audible autoplay in a cross-origin frame, which
 *   would leave a second play button to tap;
 * - `controls=0`, `iv_load_policy=3`: no chrome or annotation cards, just the
 *   movement ("Open in YouTube" gives the real player);
 * - `cc_load_policy=0`: YouTube turns captions on for a muted embed, laying text
 *   across the movement with no control to dismiss it;
 * - `rel=0`, `playsinline`: no unrelated suggestions, no iOS full-screen takeover.
 */
