...
describe("a set's detail line", () => {
  it('says reps and load', () => {
    const { page } = history();
    expect(page.detail(set('2026-07-14T16:00:00', { reps: 6, loadKg: 40 }))).toBe('6 reps · 40 kg');
  });
  ...
  it('says how far a carry went', () => {
    const { page } = history();
    expect(page.detail(set('2026-07-14T16:00:00', { reps: null, loadKg: 24, distanceM: 10 }))).toBe(
      '24 kg · 10 m',
    );
  });
