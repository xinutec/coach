    ...
    const p = pacing({
      plan: [
        suggestion({
          kind: 'warmup',
          sets: 1,
          logged: [{ reps: 10, loadKg: null, holdS: null, distanceM: null }],
        }),
        suggestion({
          exerciseId: 2,
          sets: 3,
...
describe('what comes next', () => {
  ...
  it('marks the ramp-in and not the work set that shares its movement', () => {
    ...
    t.pacing.set(
      pacing({
        plan: [suggestion({ kind: 'warmup', sets: 1 }), suggestion({ kind: 'work', sets: 3 })],
      }),
    );
...
describe('what you already did', () => {
  ...
  it('carries the per-side convention into the receipt', () => {
    ...
    expect(
      t.loggedSummary(
        suggestion({ logged: [{ reps: 9, loadKg: null, holdS: null, distanceM: null }] }),
      ),
    ).toBe('9 reps each side');
  });
});
