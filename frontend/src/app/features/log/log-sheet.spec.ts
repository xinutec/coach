...
describe('the exercise list', () => {
  it("puts today's planned movements first, in plan order", () => {
    const { sheet } = open({
      exercises: [
        exercise(1, { name: 'Alpha' }),
        exercise(2, { name: 'Beta' }),
        exercise(3, { name: 'Gamma' }),
      ],
  ...
  it('sorts the rest by what the athlete reads, not by catalog order', () => {
    ...
    expect(sheet.exercises.map((e) => e.name)).toEqual(['Ab wheel', 'Pull-up', 'Zercher squat']);
  });
...
describe('when the server objects', () => {
  function failing(err: unknown, data: Partial<LogSheetData> = {}): LogSheet {
    ...
    vi.spyOn(TestBed.inject(CoachApi), 'logSet').mockReturnValue(throwError(() => err));
    return sheet;
  }
