...
export class LocationsPage {
  ...
  readonly loading = computed(
    () =>
      !this.locationsStore.loaded() || !this.equipmentStore.loaded() || !this.placesStore.loaded(),
  );
  ...
  readonly loadableSlugs = computed(() => [...this.formEquip()].filter((s) => this.isLoadable(s)));
  ...
  platesOf(slug: string): PlateForm[] {
    return this.formPlates().filter((p) => p.equipment === slug || p.equipment === null);
  }
  ...
  removePlate(p: PlateForm): void {
    this.formPlates.set(
      this.formPlates().filter((x) => !(x.loadKg === p.loadKg && x.equipment === p.equipment)),
    );
  }
  ...
  save(): void {
    ...
    const equipmentOptions = [...this.formOptions().entries()]
      ...
      .filter((o) => o.weights.length > 0 || o.labels.length > 0 || o.barKg !== null);
