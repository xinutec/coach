...
describe("labelFor", () => {
  it("reads the accessible name in preference to the text", () => {
    const el = markup('<button aria-label="Log a set">add</button>');
    expect(labelFor(el)).toBe("Log a set");
  });
  ...
  it("strips a Material icon's ligature name out of the label", () => {
    ...
    // without stripping it every icon+label control logs as
    // "fitness_centerLibrary" and the trace is unreadable.
    const el = markup("<a><mat-icon>fitness_center</mat-icon>Library</a>");
    expect(labelFor(el)).toBe("Library");
  });
  ...
  it("finds the control a tap landed inside", () => {
    ...
    const button = markup('<button><span class="label">Today</span></button>');
    expect(labelFor(button.querySelector(".label"))).toBe("Today");
  });
  ...
  it("says nothing for a tap that missed every control", () => {
    ...
    expect(labelFor(markup("<p>Nothing logged yet.</p>"))).toBeNull();
  ...
  it("does not disturb the live DOM while reading a label", () => {
    ...
    const el = markup("<button><mat-icon>bolt</mat-icon>Today</button>");
