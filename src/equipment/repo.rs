...
/// `exercise_equipment`). A `FromRow` struct binds by name at runtime, so a
/// column list that drifts from it is a 500 the compiler cannot see; sharing the
/// fragment makes the drift impossible.
///
